...
async function branchExists(name: string): Promise<boolean> {
  try {
    const result = await Bun.$`git branch --list ${name}`.quiet();
...
- Example format: add-user-auth, fix-login-bug`;
    ...
    if (lastAttempt) {
      claudePrompt += `\n\nThe name '${lastAttempt}' is invalid or already exists. Suggest a different name.`;
    }

    let result: string;
    try {
      const proc = await Bun.$`claude --model haiku --print ${claudePrompt}`.quiet();
