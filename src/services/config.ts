...
async function readConfigFile(
  path: string,
): Promise<ConductorConfig | undefined> {
...
export function mergeConfig(
  primary: ConductorConfig | undefined,
  fallback: ConductorConfig | undefined,
): ConductorConfig {
  if (!primary && !fallback) {
    return {};
  }
  ...
  return result as ConductorConfig;
}

/**
 * Read the local .conductor/config.yml file from the current directory
 * Returns undefined if the file doesn't exist
 */
export async function readLocalConfig(): Promise<ConductorConfig | undefined> {
  return readConfigFile(LOCAL_CONFIG_PATH);
}

/**
 * Read the global ~/.conductor/config.yml file from the home directory
 * Returns undefined if the file doesn't exist
 */
export async function readHomeConfig(): Promise<ConductorConfig | undefined> {
